...
const parse = raw => raw.map(x => ({ time: Math.floor(x[0] / 1000), open: +x[1], high: +x[2], low: +x[3], close: +x[4] }));
...
export default function PracticeView({ onExit }) {
  ...
  const indicatorSeriesRef = useRef({ emas: {}, upper: null, lower: null });
  ...
  }, [candles, history, session.candleIndex, trades]);
  ...
  return <main className="container" style={{ minHeight: '80vh', position: 'relative' }}>
    ...
    <div className="practice-market-layout" style={{ display: 'flex', gap: '0.75rem', alignItems: 'stretch', minHeight: 'clamp(560px, 70vh, 900px)' }}>
      <section className="card practice-market-chart" style={{ flex: '1 1 74%', minWidth: 0, display: 'flex', flexDirection: 'column', padding: '0.75rem', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', color: 'var(--text-secondary)', fontSize: 12, paddingTop: '0.45rem' }}>{EMA_PERIODS.map(period => <span key={period} style={{ color: EMA_COLORS[period] }}>EMA {period}</span>)}<span style={{ color: '#60a5fa' }}>Bollinger Bands</span><span>Fear &amp; Greed: unavailable for historical replay</span><span>DXY: unavailable for historical replay</span></div>
      </section>
